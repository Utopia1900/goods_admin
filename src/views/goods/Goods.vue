<template>
  <div class="animated fadeIn">
    <b-tabs>
      <b-tab title="在售" @click="getGoodsList(0)">
        <div v-if="goodsList.length ===0 ">
          <span class="fa fa-info">列表中没有数据！！</span>
        </div>
        <div v-else>
          <b-table hover :items="goodsList" :fields="fields" class="responsive">
            ...
            <template slot="action" slot-scope="row">
              ...
              <span>
                <b-button variant="danger" @click="openPurcaseModal(row.item)">采购</b-button>
              </span>
            </template>
          </b-table>
        </div>
    ...
    <b-alert variant="info" dismissible :show="hasError" @dismissed="hasError=false">{{errmsg}}</b-alert>
    <b-modal id="restock_modal" centered title="填写采购信息">
        ...
        <b-list-group>
          ...
          <b-list-group-item>
            ...
            <span>
              ...
              <i>
                <input type="text" class="num_input" v-model="num">
              </i>
      ...
      <div slot="modal-footer" class="w-100">
        <b-btn size="md" class="float-right" variant="danger" @click="hideRestockModal">取消</b-btn>
        <b-btn size="md" class="float-right" variant="primary" @click="purchase(selectedGood)">确定</b-btn>
      </div>
    </b-modal>
  </div>
</template>
